    ...
    <div class="col-12">
        ...
        <div class="card card-default">
          <div class="card-header">
            <h3 class="card-title">Edit Absensi Kehadiran & Pelaksanaan Tugas Lain : <span class="badge badge-pill badge-primary">{{ $DataDiri->nama_lengkap }}</span></h3>
          ...
          <form data-route="{{ Route('ProsesEditAbsensi') }}"  data-routeback="#" id="myForm" role="form" method="POST"  accept-charset="utf-8">
          @csrf
          ...
          <input type="hidden" name="id_versi" value="{{ $id_versi }}" >
              ...
              <tr>
                <td colspan="5" class="bg-info" style="text-align: center; font-weight: bold;"><h5>Edit {{ $s_indikator->nama_indikator }}</h5></td>
              </tr>
              @php
              ...
              @endphp
                ...
                <div class="row">
                  ...
                  <div class="row">
                    ...
                    //QUERY UNTUK EDIT
                    $DataFinalAbsen = DB::table('b_final_absen')
                    ->join('b_point_absen_kehadiran','b_point_absen_kehadiran.id_detail_indikator','=','b_final_absen.id_finalDetailIndikator')
                    ->select('b_final_absen.id_finalDetailIndikator','b_final_absen.id_final_absensi')
                    ->where([['id_user','=',$id_user],['b_point_absen_kehadiran.id_aspek_fk','=',$s_sub_aspek->id_subaspek],['b_final_absen.id_versi','=',$id_versi]])
                    ->first();
                    ...
                    <div class="col-sm">
                      ...
                      <input type="hidden" name="id_final_absen[]" value="{{ $DataFinalAbsen->id_final_absensi }}">
                      ...
                      <blockquote class="quote-danger"> 
                         <select class="form-control" name="FinalIndikator[]" required="">
                            ...
                            <option value="{{ $pointIndi_s->id_detail_indikator }}" 
                              @if($pointIndi_s->id_detail_indikator == $DataFinalAbsen->id_finalDetailIndikator)
                                selected="" 
                              @endif
                              >{{ $pointIndi_s->detail_indikator }} | {{$pointIndi_s->point}} Point</option>
                            @endforeach
                          </select>
            ...
            <div class="modal-footer">
              <button type="submit" style="float: right;" class="btn btn-outline-primary btn-flat">Update Absensi & Pelaksanaan Tugas Lain</button>
            </div>
          ...
          success: function(data, Response){
           switch(data.ceks){
              case 'berhasil': /*First case */
               swal("Berhasil!", 'Berhasil Mengubah Data '+namaLengkap+' ', "success");
              break;
              default:
              break
              }
